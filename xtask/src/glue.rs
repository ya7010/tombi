...
pub mod fs2 {
    ...
    pub fn read_to_string<P: AsRef<Path>>(path: P) -> Result<String, anyhow::Error> {
        let path = path.as_ref();
...
pub fn rm_rf(path: impl AsRef<Path>) -> Result<(), anyhow::Error> {
    let path = path.as_ref();
    if !path.exists() {
        return Ok(());
    }
...
#[doc(hidden)]
pub fn run_process(cmd: String, echo: bool, stdin: Option<&[u8]>) -> Result<String, anyhow::Error> {
    run_process_inner(&cmd, echo, stdin).with_context(|| format!("process `{cmd}` failed"))
}

pub fn date_iso() -> Result<String, anyhow::Error> {
    run!("date --iso --utc")
}

fn run_process_inner(cmd: &str, echo: bool, stdin: Option<&[u8]>) -> Result<String, anyhow::Error> {
